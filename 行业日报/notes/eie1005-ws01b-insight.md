# EIE1005 · Workshop 01 (B)「To Insight」数据可视化完全笔记（v7 整合版）

> 按 note-organizer 规则：课件原文用正文色，后加讲解 / 考点 / 拓展一律用引用块（金色左边框）。本版（v7）在 v6 基础上按「整体迭代」原则整合而非堆料：各图「坑」并入选型表（§2.2）、四种图 API 并入完整代码节（§3.5），并全篇统一编号与交叉引用。融合你提供的参考（见 §参考来源）。
